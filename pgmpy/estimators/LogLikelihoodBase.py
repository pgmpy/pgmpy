...
class LogLikelihoodBase(ABC):
    ...
    def _get_state_names(self):
        ...
        for column in self.data.columns:
            if self.data[column].dtype.name == "category":
                state_names[column] = self.data[column].cat.categories.tolist()
            else:
                state_names[column] = sorted(self.data[column].unique().tolist())
        return state_names
    ...
    def score(self, model):
        if not isinstance(model, (BayesianNetwork, DiscreteBayesianNetwork)):
            raise ValueError(
                "model must be a BayesianNetwork or " "DiscreteBayesianNetwork instance"
            )
        if self.data.empty:
        ...
        if hasattr(model, "get_cpds"):
            for node in model.nodes():
                ...
                if cpd is None:
                    raise ValueError(f"Model is missing CPD for node: {node}")
        score = 0
        for node in model.nodes():
