...
class PC(BaseConstraintEstimator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
        Initializes the PC estimator.

        This method calls the initializer of the `BaseConstraintEstimator` to set up
        the data and independencies, which are common to both PC and FCI.
        """
                ...
                for Z in set(skeleton.neighbors(X)) & set(skeleton.neighbors(Y)):
                    if Z not in separating_sets[frozenset((X, Y))]:
                        if (temporal_ordering == dict()) or (
                            (
                                temporal_ordering.get(Z, float("inf"))
                                >= temporal_ordering.get(X, float("inf"))
                            )
                            and (
                                temporal_ordering.get(Z, float("inf"))
                                >= temporal_ordering.get(Y, float("inf"))
                            )
                        ):
