...
class TOPIC(BaseCausalDiscovery):
    ...
    scoring_method : str or BaseStructureScore instance, default=None
        The local score used to evaluate edge additions and prunings.
        Supported structure scores are: ``k2``, ``bdeu``, ``bds``,
        ``bic-d``, ``aic-d``, ``ll-g``, ``aic-g``, ``bic-g``,
        ``ll-cg``, ``aic-cg``, and ``bic-cg``.

        If ``None``, an appropriate default is selected automatically
