...
class VarSort(BaseOrderDiscovery):
    ...
    Sort variables by increasing marginal variance to obtain an estimated causal order, exploiting the varsortability
    pattern studied in :cite:p:`Reisach2021`. Equal variances retain their input column order. Standardizing or
    rescaling the variables can change this causal order and the recovered graph.

    The shared graph-estimation step regresses each variable on its predecessors. Absolute regression coefficients
    supply adaptive-Lasso weights, and BIC selects the Lasso penalty. Nonzero coefficients determine the DAG's edges.

    Parameters
    ----------
    estimator : sklearn-style regression estimator, default=None
        Regressor supplying adaptive weights through its ``coef_`` attribute. If None, uses
        :class:`sklearn.linear_model.LinearRegression`. The estimator is cloned before fitting; it does not affect the
    ...
    ----------
    causal_order_ : list
        Estimated causal order obtained by sorting marginal variances. This is the order used to construct the DAG
        before any conversion to a PDAG.
