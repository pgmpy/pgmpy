...
def Varsortability(X, W, tol=1e-9):
    """
    Compute the varsortability of a dataset given the true causal graph.

    Varsortability measures how well the marginal variance order of variables
    reflects the causal order. A value close to 1.0 indicates high
    varsortability, meaning SortnRegress is likely to perform well. A value
    close to 0.5 (e.g. on standardized data) indicates low varsortability.
    ...
    W : np.ndarray of shape (d, d)
        The adjacency matrix of the true causal graph, where W[i, j] != 0
        means i -> j.

    tol : float, default=1e-9
        Tolerance for counting tied variances as correctly ordered.

    Returns
    -------
    float
        Varsortability score in [0, 1].
    ...
    --------
    >>> import numpy as np
    >>> from pgmpy.causal_discovery import varsortability
    ...
    for _ in range(E.shape[0] - 1):
        n_paths += Ek.sum()
        ratio = Ek * (var / var.T)
        n_ordered_path += (ratio > 1 + tol).sum()
        n_ordered_path += 0.5 * ((ratio <= 1 + tol) * (ratio > 1 - tol)).sum()
        Ek = Ek.dot(E)
