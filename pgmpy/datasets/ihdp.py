...
def _load_ihdp_dataframe(raw_data: bytes) -> pd.DataFrame:
    df = pd.read_csv(io.BytesIO(raw_data), sep=",", header=None, names=_COLUMN_NAMES)
...
class IHDP(_BaseDataset):
    ...
    categorical_variables: ClassVar[list[str]] = ["treatment"]
    ordinal_variables: ClassVar[dict[str, list[str]]] = {}

    @classmethod
    def load_dataframe(cls) -> pd.DataFrame:
        raw_data = cls._get_raw_data("data", cls.data_url)
        return _load_ihdp_dataframe(raw_data)


class IHDP2(_BaseDataset):
    """
    References
    ----------
    .. [1] Hill, J. (2011). Bayesian Nonparametric Modeling for Causal Inference.
    ...
    data_url = base_url + "ihdp_npci_2.csv"
    ground_truth_url = None
    expert_knowledge_url = None
