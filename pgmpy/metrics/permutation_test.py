...
def permutation_based_falsification_test(
    ...
    if ci_test == "chi_square":
        ...
        raise ValueError(f"Unsupported CI test: {ci_test}. Use 'chi_square' or 'pearsonr'")
    
    logger.info(f"Starting permutation-based falsification test with {n_permutations} permutations")
    ...
    better_than_count = sum(1 for v in permutation_violations if v > lmc_violations_given)
    ...
    for node in nodes:
        ...
        for test_node in test_nodes:
            try:
                ...
                logger.debug(f"CI test failed for {node} ⊥ {test_node} | {parents}: {e}")
                continue
