...
class K2(BaseStructureScore):
    ...
    The K2 score evaluates a Bayesian network structure on fully discrete data under a
    Dirichlet prior in which all pseudo-counts are equal to 1. The local score computed by
    `local_score(variable, parents)` is

    .. math::
        \operatorname{K2}(X_i, \Pi_i) = \sum_{j=1}^{q_i} \left[ \log \Gamma(r_i)
            - \log \Gamma(N_{ij} + r_i)
            + \sum_{k=1}^{r_i} \log \Gamma(N_{ijk} + 1) \right],

    where :math:`r_i` is the cardinality of :math:`X_i`, :math:`q_i` is the number of
    parent configurations of :math:`\Pi_i`, :math:`N_{ijk}` is the count of
