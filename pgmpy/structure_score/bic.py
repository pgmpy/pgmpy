...
class BIC(LogLikelihood):
    ...
    BIC, also known as the MDL score, balances discrete log-likelihood against model
    complexity. The local score computed by `local_score(variable, parents)` is

    .. math::
        \operatorname{BIC}(X_i, \Pi_i) =
        \ell(X_i, \Pi_i) - \frac{\log n}{2} q_i (r_i - 1),

    where :math:`\ell(X_i, \Pi_i)` is the local discrete log-likelihood, :math:`n` is the
    number of rows in `self.data`, :math:`q_i` is the number of parent configurations of
    :math:`\Pi_i`, and :math:`r_i` is the cardinality of :math:`X_i`.
