...
class AICCondGauss(LogLikelihoodCondGauss):
    ...
    parameters. The local score computed by `local_score(variable, parents)` is

    .. math::
        \operatorname{AIC}(X_i, \Pi_i) = \ell(X_i, \Pi_i) - k_i,

    where :math:`\ell(X_i, \Pi_i)` is the local conditional-Gaussian log-likelihood and
    :math:`k_i` is the number of free parameters returned by `_get_num_parameters`.
