...
class CausalGraph:
    ...
    **role_kwargs : keyword arguments
    ...
    def with_adjustment_set(
        self, variables: Union[Set[Hashable], Iterable[Hashable]]
    ) -> "CausalGraph":
...
def RCT(
    ...
    return CausalGraph(graph=edges, exposure=treatment, outcome=outcome)
...
def simple_mediation_graph(
    ...
    if confounders:
        for confounder in confounders:
            edges.extend(
                [(confounder, exposure), (confounder, mediator), (confounder, outcome)]
            )

    return CausalGraph(
        graph=edges, exposure=exposure, outcome=outcome, mediator=mediator
    )


def confounded_graph(
    exposure: Hashable, outcome: Hashable, confounders: Iterable[Hashable]
) -> CausalGraph:
    """
    ...
    for confounder in confounders:
        edges.extend([(confounder, exposure), (confounder, outcome)])

    return CausalGraph(
        graph=edges, exposure=exposure, outcome=outcome, adjustment_set=set(confounders)
    )
