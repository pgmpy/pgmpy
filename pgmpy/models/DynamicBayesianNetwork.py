...
class DynamicBayesianNetwork(BayesianModel):
    def __init__(self, ebunch=None):
        ...
        The nodes that will be added to the network will have the string form.

        However the hidden nodes will compulsory have the following form.
        (node_name, time_slice)

        Here, node_name is a string while the time_slice is an integer value, which denotes
    ...
    def add_nodes_from(self, nodes):
        ...
        for node in nodes:
            if not isinstance(node, str):
                raise ValueError('the node must be of the string type')
    ...
    def add_edge(self, start, end, **kwargs):
        ...
        The edge should contain the nodes u and v
        where u and v should be of the form
        (node_name, time_slice)
        Here, node_name is a string while the time_slice is an integer value, which denotes
        the index of the time_slice that the node belongs to.
        EXAMPLE
        ...
        if not any(map(lambda x: isinstance(x[0], str) and isinstance(x[1], int), iter(edge))):
            raise AttributeError('The edge is not of a standard form')
        ...
        if edge[0][1] == edge[1][1]:
            if edge[0][1] in (0, 1):
                ...
                super(DynamicBayesianNetwork, self).add_edge(*edge_type(*new_edge))
        ...
        super(DynamicBayesianNetwork, self).add_edge(*edge_type(*edge))
    ...
    def add_edges_from(self, ebunch, **kwargs):
        ...
        List of edges: Each edge should consist of two nodes
                       ...
                       (node_name, time_slice)
                       Here, node_name is a string while the
                       time_slice is an integer value, which
                       denotes the index of the time_slice that
                       the node belongs to.
    ...
    def get_dbn_cpds(self, node=None, timeslice=None):
        ...
        >>> student.add_dbn_cpds(grade_cpd)
    ...
    def compute_initial_state(self):
        ...
        if not self.computed:
            for node in self.dbn_cpds:
                list_of_parents = super(DynamicBayesianNetwork, self).get_parents(node)
                if all(x[1] == list_of_parents[0][1] for x in list_of_parents) and list_of_parents != [] and (
                        node[0], 1 - node[1]) not in self.dbn_cpds:
                    list_of_new_parents = super(DynamicBayesianNetwork, self).get_parents((node[0], 1 - node[1]))
                ...
                if list_of_parents:
                    ...
                    self.dbn_cpds[node] = TabularCPD(node, cpd.variable_card, np.split(cpd.values, cpd.variable_card))
                super(DynamicBayesianNetwork, self).add_cpds(self.dbn_cpds[node])

            super(DynamicBayesianNetwork, self).check_model()

            if len(self.dbn_cpds) != len(super(DynamicBayesianNetwork, self).nodes()):
    ...
    def unroll(self, number_of_time_slices):
        ...
        Parameters:
        -----------
        number_of_time_slices: number of time slices that will
        ...
        while start <= number_of_time_slices:
            ...
            for node in self.dbn_cpds:
                if node[1] == 1:
                    ...
                    new_cpd_evidence = super(DynamicBayesianNetwork, self).get_parents(new_cpd_variable)
                    ...
                    super(DynamicBayesianNetwork, self).add_cpds(new_cpd)
    ...
    def remove_timeslice(self, object, number_of_time_slices):
        ...
        while start <= number_of_time_slices:
            super(DynamicBayesianNetwork, self).remove_edges_from(
                self.intra_slice(self.timestate) + self.inter_slice(self.timestate))
            list_of_nodes = list(filter(lambda x: x[1] == self.timestate, super(DynamicBayesianNetwork, self).nodes()))
            list_of_cpds = [self.dbn_cpds[node] for node in list_of_nodes]
            super(DynamicBayesianNetwork, self).remove_cpds(*list_of_cpds)
            for node in list_of_nodes:
                del self.dbn_cpds[node]
            super(DynamicBayesianNetwork, self).remove_nodes_from(list_of_nodes)
            start += 1
            self.timestate -= 1
        if self.timestate == 0:
            self.computed = False
