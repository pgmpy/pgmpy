...
class TreeAugmentedNaiveBayes(BayesianModel):
    ...
    and allows a tree structure over the independent variables to account for their interaction.
    """

    def __init__(self):
        """
        Method to initialize the `TreeAugmentedNaiveBayes` class.

        Returns
        -------
        pgmpy.models.BayesianModel instance: An instance of a Bayesian Model with the an uninitialized
            model structure.
        """
    ...
    def learn_structure(self, data, class_node, root_node=None):
        """
        Learn the model structure from a given data in the form of a pandas dataframe.
        ...
        Returns
        -------
        networkx.Graph instance: The learned model structure.
        ...
        for i in range(total_cols):
            for j in range(i+1, total_cols):
                # edge weight is the mutual info of a pair of independent variables
