...
class ContinuousNode(rv_continuous):
	...
	def _rounding_method(self, frm, to, step):
		...
		>>> std_normal._discretize(frm=-3, to=3, step=0.5, method_type='rounding')
	...
	def _lim_moment(self, u, order=1):
		...
		return integrate.quad(fun, -np.inf, u)[0] + np.power(u, order)*(1-self.cdf(u))
	...
	def discretize(self, frm, to, step, method='rounding', *args, **kwargs):
		...
		method: string or function
			...
			For more information on these methods, refer the docstrings
			of the specific methods (_rounding_method, _unbiased_method,
			_custom_method).
		*args, **kwargs:
