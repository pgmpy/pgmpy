...
class NoisyORCPD(TabularCPD):
    ...
    >>> cpd = NoisyORCPD(variable='Y', prob_values=[0.3, 0.5],
    ...                  evidence=['X1', 'X2']) # Defining a model containing NoisyORCPD.
    >>> from pgmpy.models import DiscreteBayesianNetwork
    >>> model = DiscreteBayesianNetwork([('A', 'B')]) # With nodes with no parents, we can not define a NoisyORCPD.
