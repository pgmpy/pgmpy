...
def _to_admg(causal_graph):
    """Return ``causal_graph`` as an ADMG, keeping its nodes and role assignments.

    A DAG is a semi-Markovian graph with no bidirected edges, so it is rewritten edge for edge. Anything already an
    ADMG is returned unchanged.
...
class ID(BaseFormulaIdentification):
    ...
    def _identify(self, causal_graph):
        ...
        """
        causal_graph = _to_admg(causal_graph)
