...
class Frontdoor(BaseIdentification):
    ...
    def _identify(self, causal_graph):
        ...
        possible_frontdoor_vars = set(self.observed_variables) - {exposure} - {outcome}
        ...
        for s in _powerset(possible_frontdoor_vars):
            ...
            if self.is_valid_frontdoor_adjustment_set(updated_causal_graph):
    ...
    def _is_valid_adjustment_set(causal_graph, X, Y, Z):
        ...
        causal_graph_copy.with_role("exposure", X)
    ...
    def _validate(self, causal_graph):
        ...
        unblocked_backdoor_paths_X_Z = [
            zz
            for zz in Z
            if not self._is_valid_adjustment_set(X=exposure, Y=zz, Z=set())
        ]
        ...
        for zz in Z:
            valid_backdoor_sets.append(
                self._is_valid_adjustment_set(X=zz, Y=outcome, Z=exposure)
            )
