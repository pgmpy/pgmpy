...
class MarkovChainMonteCarlo(nx.DiGraph):
    ...
    def __init__(self, ebunch=None, start_state=None):
        ...
        self.weights = {node: 0 for node in self.nodes()}
    ...
    def sample(self, size=1):
        ...
        """
        sampled = [sample_discrete(self.weights.keys(), self.weights.values())]
        for i in range(size - 1):
            values = self.edge[sampled[i]].keys()
            weights = list(map(lambda x: x['weight'], self.edge[sampled[i]].values()))
            sampled.append(sample_discrete(values, weights))
        return sampled
