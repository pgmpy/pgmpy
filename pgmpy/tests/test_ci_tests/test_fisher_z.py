...
def test_fisher_z(fisher_data):
        ...
        assert FisherZ(data=df_vstruct)("X", "Y", ["Z"], significance_level=0.05)
...
def test_fisher_z(residual_data):
...
def test_fisher_z_approx(residual_data):
        df_indep, df_dep = residual_data
        test = FisherZ(data=df_indep)
        test("X", "Y", ["Z1", "Z2", "Z3"])
        assert test.p_value_ >= 0.05
