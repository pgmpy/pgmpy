...
def test_gcm_exact(df_dep, df_indep):
    ...
    test("X", "Y", [])
    assert round(test.statistic_, 3) == pytest.approx(39.056, abs=1e-2)
    assert test.p_value_ == pytest.approx(0.0, abs=1e-2)

    # Conditional test (independent)
    test("X", "Y", ["Z1", "Z2", "Z3"])
    assert round(test.statistic_, 3) == pytest.approx(-0.349, abs=1e-2)
    assert round(test.p_value_, 4) == pytest.approx(0.7272, abs=1e-2)

    # Conditional test (dependent)
    test = GCM(data=df_dep)
    test("X", "Y", ["Z1", "Z2", "Z3"])
    assert round(test.statistic_, 3) == pytest.approx(40.967, abs=1e-2)
    assert test.p_value_ == pytest.approx(0.0, abs=1e-2)
...
def test_effect_size(df_dep, df_indep):
    test = GCM(data=df_indep)
    test("X", "Y", ["Z1", "Z2", "Z3"])
    assert test.effect_size_ == pytest.approx(0.0035, abs=1e-2)

    test = GCM(data=df_dep)
    test("X", "Y", ["Z1", "Z2", "Z3"])
    assert test.effect_size_ == pytest.approx(0.4487, abs=1e-2)
