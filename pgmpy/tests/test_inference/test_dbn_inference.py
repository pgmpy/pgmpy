...
class TestDBNInference(unittest.TestCase):
    ...
    def test_forward_inf_single_variable_with_evidence(self):
        query_result = self.dbn_inference_1.forward_inference([("Z", 1)], {("Y", 0): 0, ("Y", 1): 0})
        np_test.assert_array_almost_equal(query_result[("Z", 1)].values, np.array([0.95080214, 0.04919786]))
        query_result = self.dbn_inference_2.forward_inference([("X", 2)], {("Y", 0): 1, ("Y", 1): 0, ("Y", 2): 1})
    ...
    def test_forward_inf_multiple_variable_with_evidence(self):
        query_result = self.dbn_inference_1.forward_inference([("Z", 1), ("X", 1)], {("Y", 0): 0, ("Y", 1): 0})
        np_test.assert_array_almost_equal(query_result[("Z", 1)].values, np.array([0.95080214, 0.04919786]))
    ...
    def test_backward_inf_single_variable_with_evidence(self):
        query_result = self.dbn_inference_2.backward_inference([("X", 0)], {("Y", 0): 0, ("Y", 1): 1, ("Y", 2): 1})
        np_test.assert_array_almost_equal(query_result[("X", 0)].values, np.array([0.66594382, 0.33405618]))

        query_result = self.dbn_inference_1.backward_inference([("Z", 1)], {("Y", 0): 0, ("Y", 1): 0, ("Y", 2): 0})
