...
class TestVariableElimination(unittest.TestCase):
    ...
    def test_max_marginal_var(self):
        np_test.assert_almost_equal(self.bayesian_inference.max_marginal(["G"]), 0.6, decimal=4)

    def test_max_marginal_var1(self):
        np_test.assert_almost_equal(self.bayesian_inference.max_marginal(["G", "R"]), 0.36, decimal=4)

    def test_max_marginal_var2(self):
        np_test.assert_almost_equal(self.bayesian_inference.max_marginal(["G", "R", "A"]), 0.288, decimal=4)
    ...
    def test_invalid_state_name(self):
        ...
        with self.assertRaises(KeyError):
            self.bayesian_inference.query(variables=["J"], evidence={"A": -1}, show_progress=False)

    def test_invalid_variable_name(self):
        """Test handling of invalid variable names."""
        with self.assertRaises(ValueError):
            self.bayesian_inference.query(variables=["J"], evidence={"wrong_variable": 0}, show_progress=False)
...
class TestVariableEliminationMarkov(unittest.TestCase):
    ...
    def test_max_marginal_var1(self):
        np_test.assert_almost_equal(self.markov_inference.max_marginal(["G", "R"]), 0.1659, decimal=4)

    def test_max_marginal_var2(self):
        np_test.assert_almost_equal(self.markov_inference.max_marginal(["G", "R", "A"]), 0.1659, decimal=4)
...
class TestBeliefPropagationWithMessagePassing(unittest.TestCase):
    ...
    def test_query_error_obs_var_has_evidence(self):
        with self.assertRaises(
            ...
            self.belief_propagation.query(["B"], evidence={"A": 1}, virtual_evidence={"A": [np.array([0.1, 0.9])]})
