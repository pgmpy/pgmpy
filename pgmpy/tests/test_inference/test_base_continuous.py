...
class TestGradLogPDFGaussian(unittest.TestCase):
    ...
    def test_gradient(self):
        ...
        np_test.assert_almost_equal(grad, np.array([0.05436475, 0.49454937, 0.75465073, 0.77837868]))
        np_test.assert_almost_equal(log, -3.21046521505)
...
class TestLeapFrog(unittest.TestCase):
    ...
    def test_leapfrog_methods(self):
        ...
        np_test.assert_almost_equal(new_pos, np.array([-0.35634146, -0.25609756, -0.33]))
        np_test.assert_almost_equal(new_momentum, np.array([-1.3396624, -0.70344884, -1.16963415]))
        np_test.assert_almost_equal(new_grad, np.array([-1.0123835, 1.00139798, -0.46422764]))
        new_pos, new_momentum, new_grad = self.test_without_grad_log.get_proposed_values()
        np_test.assert_almost_equal(new_pos, np.array([-0.5001626, -0.32195122, -0.45333333]))
        np_test.assert_almost_equal(new_momentum, np.array([-1.42947981, -0.60709102, -1.21246612]))
        np_test.assert_almost_equal(new_grad, np.array([-0.89536651, 0.98893516, -0.39566396]))
...
class TestModifiedEuler(unittest.TestCase):
    ...
    def test_modified_euler_methods(self):
        new_pos, new_momentum, new_grad = self.test_with_grad_log.get_proposed_values()
        np_test.assert_almost_equal(new_pos, np.array([-1.0, 0.5]))
        np_test.assert_almost_equal(new_momentum, np.array([-2.0, 1.0]))
        np_test.assert_almost_equal(new_grad, np.array([-0.93406593, 0.08241758]))
        new_pos, new_momentum, new_grad = self.test_without_grad_log.get_proposed_values()
        np_test.assert_almost_equal(new_pos, np.array([-0.6, 0.3]))
        np_test.assert_almost_equal(new_momentum, np.array([-2.0, 1.0]))
        np_test.assert_almost_equal(new_grad, np.array([-0.56043956, 0.04945055]))
