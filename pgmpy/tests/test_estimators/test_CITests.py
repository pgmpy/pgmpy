...
class TestKernelCITests(unittest.TestCase):
    def setUp(self):
        ...
        self.df_lin_ind = pd.DataFrame(
            np.random.randn(n_samples, 3), columns=["X", "Y", "Z"]
        )
    ...
    def test_rcit_unconditional(self):
        ...
        stat, p_value = rcit(
            X="X", Y="Y", Z=[], data=self.df_lin_ind, boolean=False, seed=42
        )
        self.assertTrue(p_value > 0.05, f"Expected p_value > 0.05, got {p_value}")

        # Test non-linear dependence
        stat, p_value = rcit(
            X="X", Y="Y", Z=[], data=self.df_nonlin_dep, boolean=False, seed=42
        )
        self.assertTrue(p_value < 0.05, f"Expected p_value < 0.05, got {p_value}")
    ...
    def test_rcit_conditional(self):
        ...
        self.assertTrue(
            p_value > 0.01, f"Linear CI: Expected p_value > 0.01, got {p_value}"
        )

        # Test non-linear conditional independence
        stat, p_value = rcit(
            X="X", Y="Y", Z=["Z"], data=self.df_nonlin_cind, boolean=False, seed=42
        )
        self.assertTrue(
            p_value > 0.01, f"Non-linear CI: Expected p_value > 0.01, got {p_value}"
        )

        # Test non-linear conditional dependence
        stat, p_value = rcit(
            X="X", Y="Y", Z=["Z"], data=self.df_nonlin_cdep, boolean=False, seed=42
        )
        self.assertTrue(
            p_value < 0.01, f"Non-linear CD: Expected p_value < 0.01, got {p_value}"
        )
    ...
    def test_rcit_multiple_conditioning(self):
        ...
        self.assertTrue(
            p_value > 0.05, f"Multi-var CI: Expected p_value > 0.05, got {p_value}"
        )
    ...
    def test_rcot_conditional(self):
        ...
        self.assertTrue(
            p_value < 0.05, f"Non-linear CD: Expected p_value < 0.05, got {p_value}"
        )
    ...
    def test_kernel_tests_vs_pearsonr(self):
        ...
        tests = [(pearsonr, "pearsonr"), (rcit, "rcit"), (rcot, "rcot")]

        for test_func, test_name in tests:
            # Linear independence
            if test_name == "pearsonr":
                _, p_value = test_func(
                    X="X", Y="Y", Z=[], data=self.df_lin_ind, boolean=False
                )
            else:
                _, p_value = test_func(
                    X="X", Y="Y", Z=[], data=self.df_lin_ind, boolean=False, seed=42
            ...
            self.assertTrue(
                p_value > 0.05, f"{test_name} failed on linear independence"
            )

            # Linear conditional independence
            if test_name == "pearsonr":
                _, p_value = test_func(
                    X="X", Y="Y", Z=["Z"], data=self.df_lin_cind, boolean=False
                )
            else:
                _, p_value = test_func(
                    X="X", Y="Y", Z=["Z"], data=self.df_lin_cind, boolean=False, seed=42
    ...
    def test_kernel_tests_hyperparameters(self):
        ...
        for approx in ["lpb", "hbe"]:
            ...
            self.assertTrue(
                p_value > 0.05, f"Failed with approx={approx}: p_value={p_value}"
            )
    ...
    def test_kernel_tests_edge_cases(self):
        ...
        stat, p_value = rcit(
            X="X", Y="Y", Z=["Z"], data=df_single, boolean=False, seed=42
        )
        self.assertIsInstance(p_value, float)
        self.assertTrue(0 <= p_value <= 1)
    ...
    def test_kernel_tests_reproducibility(self):
        ...
        stat3, p3 = rcit(
            X="X", Y="Y", Z=["Z"], data=self.df_nonlin_cind, boolean=False, seed=123
        )
        self.assertNotEqual(
            stat1, stat3, "RCIT statistics should differ with different seeds"
        )
    ...
    def test_supported_tests(self):
        ...
        for test_name in supported_tests:
            ...
            self.assertIsNotNone(
                test_func, f"Failed to get test function for {test_name}"
            )
