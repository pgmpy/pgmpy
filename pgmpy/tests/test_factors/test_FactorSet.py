...
class TestFactorSet:
    ...
    def test_factorset_product(self):
        ...
        assert {self.phi1, self.phi2, self.phi3, self.phi4} == factor_set3.factors
    ...
    def test_factorset_marginalize_inplace(self):
        ...
        assert self.phi1.marginalize(["x1"], inplace=False) == phi1_equivalent_in_factor_set
        phi2_equivalent_in_factor_set = list(
            filter(lambda x: set(x.scope()) == {"x4", "x3"}, factor_set.factors)
        )[0]
        assert self.phi2.marginalize(["x1"], inplace=False) == phi2_equivalent_in_factor_set
        phi3_equivalent_in_factor_set = list(
            filter(lambda x: set(x.scope()) == {"x6", "x7"}, factor_set.factors)
        )[0]
        assert self.phi3.marginalize(["x5"], inplace=False) == phi3_equivalent_in_factor_set
        phi4_equivalent_in_factor_set = list(
            filter(lambda x: set(x.scope()) == {"x8", "x7"}, factor_set.factors)
        )[0]
        assert self.phi4.marginalize(["x5"], inplace=False) == phi4_equivalent_in_factor_set
