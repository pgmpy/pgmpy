...
def is_good_match(t1, t2):
    ...
    if len(t1_c) > 15 and t1_c in t2_c: return True
    if len(t2_c) > 15 and t2_c in t1_c: return True
...
def fetch_bibtex_from_crossref(title, authors):
    ...
    try:
        r = requests.get(bib_url, headers={"Accept": "application/x-bibtex", "User-Agent": f"pgmpy-fetch/1.0"}, timeout=REQUEST_TIMEOUT)
        r.raise_for_status()
...
def main():
    ...
    for block in entries:
        ...
        if canonical_bibtex and canonical_bibtex.startswith("@"):
            ...
            new_bibtex = re.sub(r'^@\w+\s*{[^,]+,', lambda m: m.group(0).replace(m.group(0)[m.group(0).find('{')+1:-1], key), canonical_bibtex, count=1)
